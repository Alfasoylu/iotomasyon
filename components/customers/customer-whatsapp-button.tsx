...
export function CustomerWhatsAppButton({
  ...
  if (!normalized) {
    return (
      <span className="inline-flex h-9 items-center rounded-md border border-[var(--border-subtle)] bg-[var(--surface-3)] px-4 text-sm text-[var(--text-muted)]">
  ...
  return (
    <div className="flex flex-wrap gap-2">
      <a
        ...
        rel="noreferrer"
        onClick={logContact}
        className="inline-flex h-9 items-center gap-2 rounded-md border border-[var(--ok-border)] bg-[var(--ok-dim)] px-4 text-sm font-semibold text-[var(--ok)] transition hover:brightness-110"
      >
      ...
      {templates && templates.length > 0 && (
        <div className="relative">
          <button
            type="button"
            onClick={() => setOpen((o) => !o)}
            className="inline-flex h-9 items-center gap-1.5 rounded-md border border-[var(--border-default)] bg-[var(--surface-3)] px-3 text-sm font-medium text-[var(--text-primary)] transition hover:border-[var(--border-strong)]"
          >
          ...
          {open && (
            <>
              <div
                ...
                aria-hidden
              />
              <div className="absolute right-0 top-10 z-20 max-h-80 w-72 overflow-y-auto rounded-md border border-[var(--border-default)] bg-[var(--surface-2)] p-1">
                {templates.map((t) => (
                  <button
                    key={t.id}
                    type="button"
                    onClick={() => onTemplateClick(t)}
                    className="block w-full rounded-md px-3 py-2 text-left text-sm transition hover:bg-[var(--surface-3)]"
                  >
                    <div className="font-medium text-[var(--text-primary)]">{t.name}</div>
                    {t.category && (
                      <div className="mt-0.5 text-[10px] uppercase tracking-widest text-[var(--text-muted)]">
                        {t.category}
                      </div>
                    )}
                    <div className="mt-1 line-clamp-2 text-xs text-[var(--text-secondary)]">{t.body}</div>
                  </button>
                ))}
              </div>
            </>
