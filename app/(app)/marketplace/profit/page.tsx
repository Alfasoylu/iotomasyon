 ...
 *
 * Per-listing marketplace channel profitability computed from existing
 * product cost/price data. No new DB schema required.
 *
 * Shows:
...
export default async function MarketplaceProfitPage() {
  ...
  // Compute profitability per listing
  const rows: ListingRow[] = listings.map((l) => {
    ...
    const profit = calculateProfitability({
      ...
      marketplaceCommissionOverride: p.marketplaceCommissionOverride
        ? toNum(p.marketplaceCommissionOverride)
        : null,
      packagingCost: toNum(p.packagingCost),
      vatRate: toNum(p.vatRate),
      paymentFeeRate: toNum(p.paymentFeeRate),
      returnReserveRate: toNum(p.returnReserveRate),
    });
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm leading-7 text-slate-600">
            Aktif listeleme başına pazar yeri kanalı kârlılığı — ürün maliyet ve fiyat verilerinden hesaplanmıştır.
          </p>
        </div>
      ...
      {loserRows.length > 0 && (
        <section className="space-y-3">
          ...
          <ProfitTable rows={loserRows} highlight="danger" />
        </section>
      )}
              ...
              <table className="w-full text-sm text-slate-700 border-collapse">
                <thead>
                  <tr className="border-b border-slate-100 bg-amber-50/50">
                    <th className="py-3 px-4 text-left text-xs font-semibold uppercase tracking-wide text-slate-400">
                      Platform
                    </th>
                    <th className="py-3 px-4 text-left text-xs font-semibold uppercase tracking-wide text-slate-400">
                      Ürün
                  ...
                  {noData.map((r) => (
                    <tr key={r.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                      ...
                      <td className="py-3 px-4">
                        ...
                        <p className="text-xs text-slate-600 mt-0.5 max-w-[220px] truncate">
                          {r.productName}
                        </p>
                      </td>
                      <td className="py-3 px-4">
              ...
              <table className="w-full text-sm text-slate-700 border-collapse">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/50">
                    ...
                    <th className="py-3 px-4 text-right text-xs font-semibold uppercase tracking-wide text-slate-400">
                      Stok
                    </th>
                    <th className="py-3 px-4 text-right text-xs font-semibold uppercase tracking-wide text-slate-400">
                      Satış Pot.
