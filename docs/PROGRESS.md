...
- dashboard role-based workspaces (Phase 54 Faz A+B, 2026-05-17): single /dashboard URL branches on user.role server-side; AdminWorkspace extracted with shared StatCard/LinkedStatCard components; SalesWorkspace shows pipeline (active interests by assignedToId), today tasks, recent customer activity — getSalesPipelineData() enforces no-financial-data rule at service level; admin view visually unchanged; READY dpl_AiLn79jzds4B1oJauke3LuM4jQB9
