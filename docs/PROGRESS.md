...
- GÜVENLİK: supabase RLS tüm public tablolarda açıldı (2026-06-13): 58 public tablonun tamamında RLS kapalıydı + anon/authenticated rollerinde tam CRUD yetkisi vardı → anon key (publik) ile herkes müşteri/satış verisini okuyup değiştirebilir, kimlik bilgisi kolonlarını (TrendyolConfig.apiKey, HepsiburadaConfig.password, CatalogShare.token) görebilirdi. Migration `20260613000000_enable_rls_all_public_tables` (idempotent DO bloğu, prod'a uygulandı): tüm tablolarda ENABLE ROW LEVEL SECURITY, policy yok = anon deny-all. App etkisi YOK — Prisma `postgres` (bypassrls) + Storage `service_role` (bypassrls) RLS'i bypass eder; kodda anon key/supabase-js yok. Advisor doğrulaması: rls_disabled_in_public + sensitive_columns_exposed ERROR'ları 58→0. ARTÇI: açıkta kalmış kimlik bilgileri rotate edilmeli (Trendyol apiKey, Hepsiburada password, CatalogShare token).
- trendyol satış eşleştirme ad-kodu + katalog eksiği (2026-06-13): 580 eşleşmemiş TrendyolSalesRecord (69 grup) için rematch action'a Step 3 (ad-kodu) eklendi — productName son token (", one size" sonrası) → Product.sku tam eşleşme, yalnızca tek aday; POSIX `[[:space:]]` regex (JS template backslash güvenli); RematchResult.fixedByName; page fixableCount + button güncellendi. Production'da 105 kayıt eşleştirildi (ANUNNAKIPOINTER 91, 60W-TYPE-C-SARJ-KABLOSU 11, 4929293837363 2, 490345764 1), oran %81.5→%84.8 (2653/3128), 0 belirsiz. Kalan 475'in asıl sorunu KATALOG EKSİĞİ: en büyük 2 grup AL4858 (102, hepsi son 30g) + CAPTURE4-4K-VC200 (96, hepsi son 30g) Trendyol'da aktif satan ama katalogda olmayan ürünler → eşleştirilemez, eklenmeli. Uzun kuyruk manuel bağ (/admin/trendyol-matching) gerektirir.
- ürün formu stockQuantity Entegra gate'i (2026-06-13): Codex P0 "kalan riskler" son maddesi kapatıldı — `product-actions.ts` `normalizeProductData` + `normalizeProductDataNonFinancial`'tan `stockQuantity` çıkarıldı; `createProductAction` ilk değeri bootstrap yazar, `updateProductAction` yalnızca `stockIsManuallyOwned()` (stockSource==="MANUAL" veya xmlLocked) ise yazar (XML sync'in atladığı durumlar). XML/API/IMPORT kaynaklı ürünlerde stockQuantity'ye dokunulmaz → form-load/save arası XML sync ile lost-update race kapandı. `product-form.tsx` edit modunda XML-sahipli ürünlerde "Güncel stok" alanı disabled + açıklama. Şema/migration değişikliği YOK. Phase 89 ile birlikte tüm stock source-of-truth ihlalleri çözüldü.
- katalog & satış araçları dalgası (2026-06-06): tek doğrusal yığın (`86974c2`→`74ed23c`) main'e alındı (origin/main READY). İçerik: (1) katalog yanlış uniform fiyat temizliği + `Product.catalogSortOrder` + admin sıralama (migration `20260606000000`); (2) `CompanySettings` DB + `/admin/company-settings` (DB-driven şirket profili, migration `20260606000100`); (3) katalog PDF görsel compress + `MarketplaceSalesRecord` composite index (migration `20260606000200`); (4) katalog PDF kapak yeniden tasarım (editorial); (5) favicon + apple-icon + OG/Twitter image (Soylu); (6) teklif oluşturucu aranabilir ürün combobox (`product-combobox.tsx`, 1000+ ürün); (7) DB-driven `CatalogProfile` + `/admin/catalog-profiles` (müşteri segmentine göre katalog içeriği — nalbura MUSLUK, migration `20260606000300`); (8) katalog kategori sırası korunumu + admin ↑↓ reorder; (9) import-cockpit satır görseli + marka + kategori chip; (10) stoksuz sinyali — stoğu biten ama geçmişte satılan ürünlere "Stoksuz · Acil/Sipariş" + gerçek `lifetimeMonths` + `STOKSUZ_AL` gevşetildi (`importer-cost.ts`, `test-stocksuz-signal.ts` ile doğrulandı). 4 additive migration production'a uygulandı.
...
- phase 88 satis firsatlari inline durum guncellemesi (2026-05-18): UpdateInterestForm "use client" (status+priority select, onChange → updateInterestAction, router.refresh); updateInterestAction CUSTOMERS_UPDATE gated (status/priority/followUpAt update, revalidatePath); page.tsx statik badge → UpdateInterestForm; Durum/Öncelik tek sütunda birleştirildi; browser-verified: Yeni→seçim çalıştı, tablo güncellendi; schema degisikligi yok; tsc 0 hata; commit cd6c72d; READY dpl_ERpfaSBeqxZdAbBgvKLHpMBx892P ✓
- phase 87 ekip gorev panosu (2026-05-18): /admin/task-board sayfası oluşturuldu; TASKS_ASSIGN gated; tüm açık görevler assignedToId'ye göre gruplandı (UserGroup[], openCount desc, Atanmamış en sonda); 4 KPI kart (Açık/Gecikmiş/Atanmamış/Aktif Kullanıcı); her satırda ReassignForm — useTransition + select + "Ata" butonu; assignTaskAction server action (TASKS_ASSIGN guard, Prisma update, revalidatePath); ?done=1 tamamlananları göster toggle; sidebar "Görev Panosu" (TASKS_ASSIGN); schema değişikliği yok; tsc 0 hata; commit f1ad15e; READY dpl_8jWApfcCLstDjWLXd7qufexAHp2D; browser-verified 2026-05-18 ✓
- phase 86 satis firsatlari motoru (2026-05-18): /admin/sales-opportunities sayfası oluşturuldu; ProductInterest kayıtları ürüne göre gruplandı (WON/LOST/CANCELLED hariç); 5 KPI kart; 4 filtre sekmesi (Tümü/Stok Var/Takip/Stok Bekliyor); expand=productId ile genişletilebilir satır; Stok:1000 Anunnaki Pointer 2 müşteri / Stok:0 Baofeng 1 müşteri görüldü; tab=hot filtresi Baofeng'i doğru hariç tuttu; CUSTOMERS_READ gated; schema değişikliği yok; tsc 0 hata; commit cbf2090; READY dpl_27nV8A7ySRz1YbCP1pxB27YokdnW; browser-verified 2026-05-18 ✓
- phase 85 ithalat cockpiti satin alma siparisi koprusu (2026-05-18): recommendedQty (90-gun hedef) cockpit Row tipine eklendi; orderCandidates + orderHref URL; "Siparis Olustur (N)" emerald buton; purchase-orders/new from=cockpit kolu + banner; 7 AL-urun butonu gorundu, from=cockpit&items=... ile navigate edildi; tsc 0 hata; commit 1fb44c3; READY dpl_F8TJx3MjjGG5nSss6jo6uSD1fLbX; browser-verified ✓
- phase 84 trendyol eslestirme sayfasinda hizli urun baglantisi (2026-05-18): LinkProductButton modal — arama input (300ms debounce, searchProductsAction) + linkTrendyolGroupAction (updateMany productId=NULL grubuna); 62 buton gosterildi, modal acildi, "sarj" 12 sonuc, tsc 0 hata; commit 2b12832; READY dpl_21ZAQyB3WivZUuyYzH2cXQQrffQv; browser-verified ✓
- phase 83 trendyol satis eslestirme yonetimi (2026-05-18): 1.175 eslesmemis TrendyolSalesRecord'dan 621'i LOWER(merchantSku)=LOWER(Product.sku) SQL UPDATE ile otomatik eslesti; eslesen %23→%64; kalan 554 (62 grup) manuel katalog guncellemesi gerektirir; rematch-action.ts server action + trendyol-matching admin sayfasi + RematchButton client component; sidebar Satis Eslestirme girisi; schema degisikligi yok; tsc 0 hata; commit aaa4ec4; READY dpl_GmmH2fVJGJsV4xA3QPndyYNVaJjx; browser-verified ✓
- phase 82 ithalatci gorunumu satir-ici duzenleme + eksik veri chipleri + skor (2026-05-18): InlineEditNumber bileseni (tikla-input, Enter/blur kaydet, Escape iptal); saveInlineField() PATCH + recalcProduct() client-side anlık hesap; API yaniti {products,usdTryRate,rmbUsdRate}; Alas/Agirlik/Gumruk tabloda mavi baslikli duzenlenebilir sutunlar; getMissingFields() Durum hucresi altinda specifik eksik alan chipleri (Alas RMB/Agirlik/T.Fiyat/Satis Yok); Skor buyuk numara + mini bar + baslik tiklamasiyla sort; colSpan 17→19; tsc 0 hata; commit a7c5a32; READY dpl_4SooZD6dtDdH5ujjZ7sFxbS5rgjj; browser-verified: inline edit 18.90Y kaydedildi, Maliyet Eksik 735→733, Skor sort 87→73 ✓
- phase 81 ithalatci gorunumu siparis koprusu (2026-05-18): importer view filtre cubuguna "Siparis Olustur (N)" emerald buton eklendi; recommendedQty>0 urunler items=id:qty URL param olarak encode; purchase-orders/new sayfasi from=importer branch parse; importerItemMap + importerSuggestions suggestedQty override; emerald info banner; schema degisikligi yok; tsc 0 hata; commit 34837bc; READY dpl_CKkYeSr2f25L7rsGEBNaEsUccyqU; browser-verified: 2 urun oneri ile form acildi, banner gorundu ✓
- phase 80 ithalatci gorunumu hizli duzenleme modali (2026-05-18): importer view tablosuna ✏ buton sütunu eklendi (762 buton, her satır için); ImportQuickEdit modal: sourceCostRmb, weightKg, customsRatePct, importPaymentFeePct (number input) + shippingMethodPref (3-buton toggle); PATCH /api/products/[id]/import-fields ADMIN-only endpoint (parseDecimalField partial update, shippingMethodPref doğrulaması, P2025 yönetimi); onSave() optimistik güncelleme hasCost dahil; colSpan 16→17; tsc 0 hata; commit 72d74b0; READY dpl_DxRQD6HVXLRbWXsR1s7JkWixmGy9; browser-verified: 762 buton, PAM8610 modal açıldı ve kaydedildi ✓
- phase 79 ithalatci gorunumu (2026-05-18): lib/importer-cost.ts saf hesaplama (calcImportCost/calcRevenue/calcProfit/calcStockDays/calcDecisionLabel/calcHealthScore/allocateBudget); /api/products/importer-view ADMIN-only endpoint (rol kontrolü, T30G velocity, exchange rates, tam hesaplama); ImporterViewClient 16-sütun tablo + 6 özet kart + bütçe parametreleri + filtre/sort; products/page.tsx ?view=importer toggle; Prisma client yeniden oluşturuldu (marketplacePrices); tsc 0 hata; commit 59433f9; READY dpl_AHpCzDDTJL5kEJr9tN1y5oSBbbu1; browser-verified 762 ürün / $12,445 stok maliyeti / 2 sipariş önerisi / 19 yüksek ROI ✓
- phase 78 toplu ithalat verisi xlsx (2026-05-18): xlsx@0.18.5 eklendi; bulk-export .xlsx (SheetJS, mavi başlık, sarı eksik hücre); bulk-import hem .xlsx hem .csv; ProductBulkButtons header'da; CODEX_INSTRUCTIONS stok mimarisi; tsc clean; READY dpl_6Qh1AEHrAKf6GQpm5QumWdNgK6NA ✓
- phase 75 ikincil xml feed desteği (2026-05-18): XmlSyncSource'a secondaryUrl String? alanı eklendi; runSync 4. parametre secondaryUrl; iki feed paralel fetch edilip SKU bazında merge ediliyor — birincil feed öncelikli, ikincil eksik SKU + null alanları tamamlar; stok için daha güncel date_change öncelikli; mergeXmlRecords + mergeXmlRecord yardımcı fonksiyonlar; xml-sync-form'a ikincil URL input alanı; cron route güncellendi; migration additive; DB applied; commit b587f3a; Vercel deploy pending
- phase 74 urun listesi durum filtresi (2026-05-18): /products sayfasına kârlılık durum filtre pilleri eklendi — Tümü/Mükemmel/İyi/Düşük/Zarar/Veri Yok; profit+health pre-computed allRows Map; filteredRows URL param durumFilter ile; count badges per pill; active pill colored background; footer "X gösteriliyor (Y toplam)"; durumHref() q/status/stock/sort'u koruyarak durum param ekliyor; no schema change; tsc 0 yeni hata; commit a248b81 ✓
- phase 73 kilitli sermaye dagilim analizi (2026-05-18): /admin/capital sayfasına "Kilitli sermaye dağılımı" bölümü eklendi — öneriler tablosunun altında; mevcut productsWithScore datasından hesaplandı (yeni DB sorgusu yok); top-20 ürün stok değerine göre azalan sırada; stockQty × landedCost değeri, toplam içindeki % pay, amber bar (>=10%); tfoot toplam kilitli değer; BreakdownRow tipi ile TSC clean; no schema change; commit 3c93715; browser-verified 2026-05-18 ✓
